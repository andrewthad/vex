...
type MutableVector = MutableIntVector

newtype IntVector :: GHC.Nat -> Type where
  IntVector :: PrimArray Int -> IntVector n

newtype MutableIntVector :: Type -> GHC.Nat -> Type where
  MutableIntVector :: MutablePrimArray s Int -> MutableIntVector s n

new :: Nat n -> ST s (MutableVector s n)
{-# INLINE new #-}
-- This is a core operation. Be careful though because the memory
-- is uninitialized.
new (Nat n) = fmap MutableIntVector (PM.newPrimArray n)

replicateM :: Nat n -> Int -> ST s (MutableVector s n)
-- This is a core operation. It does not need to be
replicateM (Nat n) i = fmap MutableIntVector $ do
  arr <- PM.newPrimArray n
  PM.setPrimArray arr 0 n i
  pure arr

singleton :: Int -> Vector 1
{-# INLINE singleton #-}
singleton x = IntVector $ runPrimArrayST $ do
...
index Lt (IntVector arr) (Nat i) = PM.indexPrimArray arr i
...
{-# INLINE read #-}
-- this is a core operation
read Lt (MutableIntVector arr) (Nat i) = PM.readPrimArray arr i
...
{-# INLINE write #-}
-- this is a core operation
write Lt (MutableIntVector arr) (Nat i) x = PM.writePrimArray arr i x
...
copy ::
  ...
  -> ST s ()
-- this is a core operation
copy Lt Lt (MutableIntVector dst) (Nat doff) (IntVector src) (Nat soff) (Nat len) =
...
{-# INLINE unsafeFreeze #-}
-- this is a core operation
unsafeFreeze (MutableIntVector marr) = do
  fmap IntVector (PM.unsafeFreezePrimArray marr)

length :: Vector n -> Nat n
{-# INLINE length #-}
-- this is a core operation
length (IntVector x) = Nat (PM.sizeofPrimArray x)
